...
class TotalPedidosRecebidosRepository(private val jdbcTemplate: NamedParameterJdbcTemplate) {
    ...
    fun getPedidos(periodo: String? = null, codRepresentante: Int? = null,
        ...
        sql.append("(select if(count(*)<>1,'N�o','') as pertence_regiao from regiao a ")
        ...
        sql.append("(select if(count(*)=0,'N�o','') as pertence_regiao from regiao a ")
        sql.append("where a.cod_cidade = cliente.cod_cidade ")
        sql.append("and a.`cod_representante` = representante.cod_representante) ")
        ...
        sql.append("(select if(count(*)=1,'Sim','N�o') from pedido ")
